...
public class SPPBluetooth {
    ...
    public void setupService() {
        mChatService = new SPPBluetoothService(/*mContext, */mHandler);
    }
...
//    }
    ...
    private final Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case SPPBluetoothState.MESSAGE_WRITE:
                    break;
                case SPPBluetoothState.MESSAGE_READ:
                    ...
                    break;
                case SPPBluetoothState.MESSAGE_DEVICE_NAME:
                    mDeviceName = msg.getData().getString(SPPBluetoothState.DEVICE_NAME);
                    mDeviceAddress = msg.getData().getString(SPPBluetoothState.DEVICE_ADDRESS);
                    if(mBluetoothConnectionListener != null)
                        mBluetoothConnectionListener.onDeviceConnected(mDeviceName, mDeviceAddress);
                    isConnected = true;
                    break;
                case SPPBluetoothState.MESSAGE_TOAST:
                    Toast.makeText(mContext, msg.getData().getString(SPPBluetoothState.TOAST)
                            , Toast.LENGTH_SHORT).show();
                    break;
                case SPPBluetoothState.MESSAGE_STATE_CHANGE:
                    if(mBluetoothStateListener != null)
                        mBluetoothStateListener.onServiceStateChanged(msg.arg1);
                    if(isConnected && msg.arg1 != SPPBluetoothState.STATE_CONNECTED) {
                        if(mBluetoothConnectionListener != null)
                    ...
                    if(!isConnecting && msg.arg1 == SPPBluetoothState.STATE_CONNECTING) {
                        isConnecting = true;
                    } else if(isConnecting) {
                        if(msg.arg1 != SPPBluetoothState.STATE_CONNECTED) {
                            if(mBluetoothConnectionListener != null)
    ...
    public void connect(Intent data) {
        String address = data.getExtras().getString(SPPBluetoothState.EXTRA_DEVICE_ADDRESS);
    ...
    public void disconnect() {
        if(mChatService != null) {
            ...
            if(mChatService.getState() == SPPBluetoothState.STATE_NONE) {
                isServiceRunning = true;
                mChatService.start(/*SPPBluetooth.this.isAndroid*/);
            }
        }
    }
    ...
    public void send(byte[] data, boolean CRLF) {
        if(mChatService.getState() == SPPBluetoothState.STATE_CONNECTED) {
            if(CRLF) {
